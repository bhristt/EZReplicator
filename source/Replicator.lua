...
function ReplicatorFunctions:GetClientSignal(signalName: string): BindableEvent
	if IS_SERVER then
		local pself = private[self]
		local signalListeners = pself.SignalListeners
		local listener = signalListeners[signalName]
		if listener == nil then
			Func.StoreNewBindable(signalListeners, signalName)
			listener = signalListeners[signalName]
		end
		return listener
	else
...
--// returned by this function is also fired
function ReplicatorFunctions:GetServerSignal(signalName: string): BindableEvent
	if not IS_SERVER then
		local pself = private[self]
		local signalListeners = pself.SignalListeners
		local listener = signalListeners[signalName]
